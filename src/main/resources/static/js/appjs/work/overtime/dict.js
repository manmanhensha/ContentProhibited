...
function selectLoad() {
	var html = "";
	$.ajax({
		url : '/work/overtime/name',
		success : function(data) {
			//加载数据
			for (var i = 0; i < data.length; i++) {
				html += '<option value="' + data[i].workNum + '">' + data[i].name + '</option>'
			}
		...
		.bootstrapTable(
			{
				...
				columns : [
                    ...
                    },
					{
						field : 'state',
						title : '状态',
                        formatter: function (value, row, index) { if (value == 1) { return "未处理"; } else { return "已处理"; } }
					},
					{
						field : 'timeLength',
						title : '时长（小时）'
